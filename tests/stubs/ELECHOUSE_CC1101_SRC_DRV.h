...
class ELECHOUSE_CC1101 {
 public:
  ...
  void SpiStrobe(uint8_t strobe) {
    strobe_history.push_back(strobe);
    if (strobe == CC1101_SIDLE) {
      marcstate = 0x01;
    } else if (strobe == CC1101_SRX) {
      marcstate = 0x0D; // MARCSTATE_RX
    }
  }
  uint8_t SpiReadStatus(uint8_t addr) {
    if (addr == CC1101_VERSION) return version;
    if (addr == CC1101_RXBYTES) return rxbytes;
  ...
  void SpiWriteBurstReg(uint8_t, uint8_t*, size_t) {}
  ...
  size_t rx_fifo_pos{0};
  int rssi{0};
  int lqi{0};
  std::vector<uint8_t> strobe_history;
};
