...
static bool ok = registerDriver([](DriverInfo &di) {
  ...
  di.addDetection(0x8614 /*APT?*/, 0x07,
                  0x05); // Older version of telegram that is not understood!
...
void Driver::processContent(Telegram *t) {
  ...
  // Each register is identified with a single byte after which the content
  // follows. For example, the total volume is marked by 0x10 followed by 4
  // bytes.
  ...
  // next 3B : Faults - In default frame example: please see description of 0x01
  // register

  bool register10_found = false;
  ...
  while (i < content.size()) {
    ...
    if (size == -1 || i + size > content.size()) {
      ...
      if (t->beingAnalyzed() == false) {
        if (size == -1) {
          warning("(apator162) telegram contains a register (%02x) with "
                  "unknown size.\n"
          ...
          warning("(apator162) telegram decoding fails since last register "
                  "(%02x size %d) does not\n"
    ...
    if (c == 0x10 && size == 4 && i + size < content.size()) {
      register10_found = true;
      // We found the register representing the total
      string total;
      strprintf(&total, "%02x%02x%02x%02x", content[i + 0], content[i + 1],
                content[i + 2], content[i + 3]);
      int offset = i - 1 + t->header_size;
      vendor_values["0413"] = {offset, DVEntry(offset, DifVifKey("0413"),
                                               MeasurementType::Instantaneous,
                                               0x13, {}, {}, 0, 0, 0, total)};
      double total_water_consumption_m3{};
      extractDVdouble(&vendor_values, "0413", &offset,
                      &total_water_consumption_m3);
      total = "*** 10-" + total + " total consumption (%f m3)";
      t->addSpecialExplanation(offset, 4, KindOfData::CONTENT,
                               Understanding::FULL, total.c_str(),
                               total_water_consumption_m3);
      ...
      t->addSpecialExplanation(i - 1 + t->header_size, size,
                               KindOfData::CONTENT, Understanding::NONE,
                               msg.c_str());
    }
    i += size;
  }
  ...
  if (!register10_found) {
    warning("(apator162) register 0x10 not found\n");
  }
}
...
int Driver::registerSize(int c) {
  ...
  case 0x00:
    return 4; // Date
  case 0x01:
    return 3; // Faults - In default frame f.ex. 0F 09 4D A1 97 18 02 00 -> 18
              // 02 00 -> 00 02 18 -> 0x0218
  ...
  case 0x42:
    return 4; // Energy
  case 0x43:
    return 2; // Life days - In default frame f.ex. 43 6E 0A -> 2670 days from
              // first run
